...
编辑顺序：先让普通人想读，再让人读得顺，最后用必要边界防止误导。围绕真实问题、困难、行动和发现讲故事；陌生概念先解释再命名；默认完整正文3000—4000字、最长4000字，短题材不凑字数，用户明确要求其他篇幅时优先遵从，不虚构情节。认知科学和传播学依据及迁移限制见 [传播原则](docs/communication_principles.md)。

## 0.5.0：阅读体验与图文写作

新增[阅读体验与图文作文原则](docs/reader_experience.md)，并同步技能入口、活跃规则及独立读者任务：

- 降低每句话的新信息负担，熟悉的生活内容也占用理解精力；允许围绕同一线索停顿、回看和适量复述。
- 用持续的问题和研究行动推进叙事，少用提纲、密集标题和卡片；不为结构化而作图。
- 配置相关且有吸引力的真实图片；解释动态过程时按需尝试GIF，提供静态回退并实际检查。
- 讲清基础科学与普通人生活或认识世界的直接、间接联系，区分学科长期贡献和本次已经实现的成果。
- `check`默认检查正文4000汉字上限，保留用户明确篇幅要求的覆盖字段；超长审阅稿仍可导出。不把字数达标当作易读证明。

这些是基于用户反馈的编辑要求，不是新的文献效果结论，也不代表真人阅读效果已经验证。测试覆盖默认上限边界、选读统计及用户篇幅覆盖。
...
本项目原创代码、技能、模板、规则与文档采用 [MIT 许可证](LICENSE)。任何人都可以使用、复制、修改、再分发和商用，版权归原作者所有，复制或分发全部或实质部分时须保留版权声明和许可文本；不要求在宣传或使用界面中额外署名。欢迎 Fork、改进或提交 Pull Request，不要求将修改贡献回来。软件按原样提供，不作担保。第三方文献和图片仍归各自权利人，不适用本项目许可。见 [隐私与许可](docs/privacy-and-licensing.md)。
